namespace Frida.Droidy.Injector {
	...
	private class Session : Object {
		...
		private async GadgetDetails inject_gadget (Cancellable? cancellable) throws Error, IOError {
			string so_path_shared = "/data/local/tmp/gadget.so";
			string so_path_app = "/data/data/" + package + "/gadget.so";
			string config_path_shared = "/data/local/tmp/gadget.config";
			...
			yield Frida.Droidy.ShellCommand.run (
				"am set-debug-app -w --persistent '%s'".printf (package), device_serial, cancellable
			);

			yield Frida.Droidy.ShellCommand.run (
				"am force-stop '%s'".printf (package), device_serial, cancellable
			);
